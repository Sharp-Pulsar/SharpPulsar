...
namespace SharpPulsar.Protocol.Circe
{
	...
	/// </pre>
	/// 
	/// where h corresponds to the <seealso cref="StatelessLongHash.calculate calculate"/>
	...
	/// </para>
	/// </summary>
	public interface IncrementalLongHash : StatelessLongHash
	{
		...
		long Resume(long Current, sbyte[] Input);
		...
		long Resume(long Current, sbyte[] Input, int Index, int Length);
		...
		/// <returns> the output of the hash function for the concatenated input </returns>
		long Resume(long Current, IByteBuffer Input);
