    ...
    Target Pack => _ => _
      ...
      .Executes(() =>
      {
          ...
          DotNetPack(s => s
              ...
              .SetVersion($"2.1.0")
              .SetPackageReleaseNotes($"[BREAKING CHANGES]{Environment.NewLine} Affected API: EventTime, DeliverAt and DeliverAfter!{Environment.NewLine} Change EventTime, DeliverAt and DeliverAfter parameters from long to DateTime, DateTimeOffSet and TimeSpan respectively to prevent users from passing in values not representing milliseconds, which may lead to unexpected behavior!{Environment.NewLine} Assumption removed!")
