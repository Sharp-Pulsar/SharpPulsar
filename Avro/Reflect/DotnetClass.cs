    ...
    public class DotnetClass
    {
        ...
        public DotnetClass(Type t, RecordSchema r, ClassCache cache)
        {
            ...
            foreach (var f in r.Fields)
            {
                ...
                PropertyInfo prop = this.GetPropertyInfo(f);
                ...
                foreach (var attr in prop.GetCustomAttributes(true))
                {
                    ...
                    {
                        hasAttribute = true;
                        this._propertyMap.TryAdd(f.Name, new DotnetProperty(prop, f.Schema.Tag, avroAttr.Converter, cache));
                        break;
                    }
                }
                ...
                if (!hasAttribute)
                {
                    this._propertyMap.TryAdd(f.Name, new DotnetProperty(prop, f.Schema.Tag, cache));
                }
            }
        }
        ...
        {
            var prop = this._type.GetProperty(f.Name);
            if (prop != null)
            {
                return prop;
            }
            ...
            throw new AvroException($"Class {this._type.Name} doesnt contain property {f.Name}");
        }
        ...
        {
            DotnetProperty p;
            if (!this._propertyMap.TryGetValue(f.Name, out p))
            {
