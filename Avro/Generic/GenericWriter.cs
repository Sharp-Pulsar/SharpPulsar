...
namespace Avro.Generic
{
    ...
    public class GenericWriter<T> : DatumWriter<T>
    {
        ...
        /// <inheritdoc/>
        public Schema Schema { get { return this.writer.Schema; } }
    ...
    {
        /// <summary>
        /// Gets schema that this object uses to write datum.
        /// </summary>
        ...
        {
            this.Write(this.Schema, value, encoder);
        }
        ...
        public virtual void Write(Schema schema, object value, Encoder encoder)
        {
            switch (schema.Tag)
            {
                case Schema.Type.Null:
                    this.WriteNull(value, encoder);
                    break;
                case Schema.Type.Boolean:
                    this.Write<bool>(value, schema.Tag, encoder.WriteBoolean);
                    break;
                case Schema.Type.Int:
                    this.Write<int>(value, schema.Tag, encoder.WriteInt);
                    break;
                case Schema.Type.Long:
                    this.Write<long>(value, schema.Tag, encoder.WriteLong);
                    break;
                case Schema.Type.Float:
                    this.Write<float>(value, schema.Tag, encoder.WriteFloat);
                    break;
                case Schema.Type.Double:
                    this.Write<double>(value, schema.Tag, encoder.WriteDouble);
                    break;
                case Schema.Type.String:
                    this.Write<string>(value, schema.Tag, encoder.WriteString);
                    break;
                case Schema.Type.Bytes:
                    this.Write<byte[]>(value, schema.Tag, encoder.WriteBytes);
                    break;
                case Schema.Type.Record:
                case Schema.Type.Error:
                    this.WriteRecord(schema as RecordSchema, value, encoder);
                    break;
                case Schema.Type.Enumeration:
                    this.WriteEnum(schema as EnumSchema, value, encoder);
                    break;
                case Schema.Type.Fixed:
                    this.WriteFixed(schema as FixedSchema, value, encoder);
                    break;
                case Schema.Type.Array:
                    this.WriteArray(schema as ArraySchema, value, encoder);
                    break;
                case Schema.Type.Map:
                    this.WriteMap(schema as MapSchema, value, encoder);
                    break;
                case Schema.Type.Union:
                    this.WriteUnion(schema as UnionSchema, value, encoder);
                    break;
                case Schema.Type.Logical:
                    this.WriteLogical(schema as LogicalSchema, value, encoder);
                    break;
                default:
                    this.Error(schema, value);
                    break;
            }
        }
        ...
        protected virtual void WriteNull(object value, Encoder encoder)
        {
            if (value != null)
            {
                throw this.TypeMismatch(value, "null", "null");
            }
        }
        ...
        protected virtual void Write<T>(object value, Schema.Type tag, Writer<T> writer)
        {
            if (!(value is T))
            {
                throw this.TypeMismatch(value, tag.ToString(), typeof(T).ToString());
            }
        ...
        {
            this.EnsureRecordObject(schema, value);
            foreach (Field field in schema)
            {
                try
                {
                    object obj = this.GetField(value, field.Name, field.Pos);
                    this.Write(field.Schema, obj, encoder);
                }
                catch (Exception ex)
                {
        ...
        protected virtual void EnsureRecordObject(RecordSchema s, object value)
        {
            ...
            {
                throw this.TypeMismatch(value, "record", "GenericRecord");
            }
        }
        ...
        protected virtual void WriteEnum(EnumSchema es, object value, Encoder encoder)
        {
            ...
            {
                throw this.TypeMismatch(value, "enum", "GenericEnum");
            }
        ...
        {
            this.EnsureArrayObject(value);
            long l = this.GetArrayLength(value);
            encoder.WriteArrayStart();
            encoder.SetItemCount(l);
            for (long i = 0; i < l; i++)
            {
                encoder.StartItem();
                this.Write(schema.ItemSchema, this.GetArrayElement(value, i), encoder);
            }
        ...
        protected virtual void EnsureArrayObject(object value)
        {
            ...
            {
                throw this.TypeMismatch(value, "array", "Array");
            }
        }
        ...
        {
            this.EnsureMapObject(value);
            IDictionary<string, object> vv = (IDictionary<string, object>)value;
            encoder.WriteMapStart();
            encoder.SetItemCount(this.GetMapSize(value));
            foreach (KeyValuePair<string, object> obj in this.GetMapValues(vv))
            {
                encoder.StartItem();
                encoder.WriteString(obj.Key);
                this.Write(schema.ValueSchema, obj.Value, encoder);
            }
            encoder.WriteMapEnd();
        }
        ...
        protected virtual void EnsureMapObject(object value)
        {
            ...
            {
                throw this.TypeMismatch(value, "map", "IDictionary<string, object>");
            }
        }
        ...
        {
            int index = this.ResolveUnion(us, value);
            encoder.WriteUnionIndex(index);
            this.Write(us[index], value, encoder);
        }
        ...
        {
            this.Write(ls.BaseSchema, ls.LogicalType.ConvertToBaseValue(value, ls), encoder);
        }
        ...
        protected virtual void WriteFixed(FixedSchema es, object value, Encoder encoder)
        {
            ...
            {
                throw this.TypeMismatch(value, "fixed", "GenericFixed");
            }
