...
namespace SharpPulsar.Util.Circe
{
    public class Crc32CIntChecksum
	{
		...
		static Crc32CIntChecksum()
		{
			...
			{
				Crc32CHash = (new Crc32CSse42Provider()).GetIncrementalInt(CrcParameters.Crc32);
				Log.LogInformation("SSE4.2 CRC32C provider initialized");
			}
			else
            {
		...
		public static int ComputeChecksum(IByteBuffer payload)
		{
			...
			{
				return Crc32CHash.Calculate(payload.GetPinnableMemoryAddress() + payload.ReaderIndex, payload.ReadableBytes);
			}
		...
		public static int ResumeChecksum(int previousChecksum, IByteBuffer payload)
		{
			if (payload.HasMemoryAddress && (Crc32CHash is Circe.Sse42Crc32C))
			{
				return Crc32CHash.Resume(previousChecksum, payload.GetPinnableMemoryAddress() + payload.ReaderIndex, payload.ReadableBytes);
			}
			else if (payload.HasArray)
			{
