...
public class Crc32CLongChecksum
	{
		...
		{
			if (Circe.Sse42Crc32C.Supported)
			{
				Crc32CHash = (new Crc32CSse42Provider()).GetIncrementalInt(CrcParameters.Crc32C);
				if (Log.IsEnabled(LogLevel.Debug))
				{
					Log.LogDebug("SSE4.2 CRC32C provider initialized");
				}
			}
			else
			{
				Crc32CHash = (new Circe.StandardCrcProvider()).GetIncrementalInt(CrcParameters.Crc32C);
		...
		{
			int crc;
			if (payload.HasMemoryAddress && (Crc32CHash is Circe.Sse42Crc32C))
			{
				crc = Crc32CHash.Calculate(payload.GetPinnableMemoryAddress() + payload.ReaderIndex, payload.ReadableBytes);
			}
		...
		public static long ResumeChecksum(long previousChecksum, IByteBuffer payload)
		{
			...
			if (payload.HasMemoryAddress && (Crc32CHash is Circe.Sse42Crc32C))
			{
				crc = Crc32CHash.Resume(crc, payload.GetPinnableMemoryAddress() + payload.ReaderIndex, payload.ReadableBytes);
			}
			else if (payload.HasArray)
			{
