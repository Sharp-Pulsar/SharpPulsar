...
{
    public sealed class ClientCnx : UntypedActor, IWithTimers
    {
        ...
        public ClientCnx(ClientConfigurationData conf, DnsEndPoint endPoint, int protocolVersion, TaskCompletionSource<ConnectionOpened> connectionFuture, string targetBroker)
        {
            ...
            _handler = new ClientHandler(null, conf, new MultithreadEventLoopGroup(conf.NumIoThreads));
                ...
                case Payload p:
                    {
                        ...
                        {
                            case "NewLookup":
                                _handler.NewLookup(p.Bytes, p.RequestId);
                                break;
                            case "NewAckForReceipt":
                                _handler.NewAckForReceipt(p.Bytes, p.RequestId);
                                break;
                            case "NewGetTopicsOfNamespaceRequest":
                                _handler.NewGetTopicsOfNamespace(p.Bytes, p.RequestId);
                                break;
                            case "SendGetLastMessageId":
                                _handler.SendGetLastMessageId(p.Bytes, p.RequestId);
                                break;
                            case "SendGetRawSchema":
                                _handler.SendGetRawSchema(p.Bytes, p.RequestId);
                                break;
                            case "SendGetOrCreateSchema":
                                _handler.SendGetOrCreateSchema(p.Bytes, p.RequestId);
                                break;
                            ...
                            case "NewAddSubscriptionToTxn":
                                _handler.HandleAddSubscriptionToTxn(p.);
                                break;
                            ...
                            default:
                                _handler.SendRequest(p.Bytes, p.RequestId);
                                break;
                        }
                        break;
                    }
                case CommandConnected connected:
                    {
                        _handler.HandleConnected(connected); 
                        break;
                    }
                case RegisterProducer producer:
                    {
                        _handler.RegisterProducer<RegisterProducer>(producer.ProducerId, producer.Producer);
                        break;
                    }
                case RegisterConsumer consumer:
                    {
                        _handler.RegisterConsumer<RegisterConsumer>(consumer.ConsumerId, consumer.Consumer);
                        break;
                    }
                case RemoveProducer removeProducer:
                    {
                        _handler.RemoveProducer(removeProducer.ProducerId);
                        break;
                    }
                case RemoveConsumer removeConsumer:
                    {
                        _handler.RemoveConsumer(removeConsumer.ConsumerId);
                        break;
                    }
                case Close close:
                    {
        ...
        public class ClientHandler : PulsarHandler
        {
            ...
            private readonly Dictionary<long, (AbstractByteBuffer Message, IActorRef Requester)> _pendingRequests = new Dictionary<long, (AbstractByteBuffer Message, IActorRef Requester)>();
            ...
            private readonly Dictionary<long, IActorRef> _watcher = new Dictionary<long, IActorRef>();
            ...
            private IAuthenticationDataProvider _authenticationDataProvider;
            internal ClientHandler(InstrumentProvider instrumentProvider,
            ...
            internal ClientHandler(InstrumentProvider instrumentProvider, ClientConfigurationData conf, IEventLoopGroup eventLoopGroup,
                         int protocolVersion) : base(conf.KeepAliveIntervalSeconds, TimeUnit.TimeUnit.SECONDS)
            {
                ...
                _pendingLookupRequestSemaphore = new Semaphore(conf.ConcurrentLookupRequest, short.MaxValue);
                _maxLookupRequestSemaphore =
                        new Semaphore(conf.MaxLookupRequest - conf.ConcurrentLookupRequest, short.MaxValue);
            ...
            public override void ChannelActive(IChannelHandlerContext ctx)
            {
                ...
                _ctx = ctx;
                _connectionsOpenedCounter.GetAndIncrement();
            ...
            public override void ChannelInactive(IChannelHandlerContext ctx)
            {
                ...
                _connectionsClosedCounter.GetAndIncrement();
                _lastDisconnectedTimestamp = DateTimeHelper.CurrentUnixTimeMillis();
                _log.Info("{} Disconnected", ctx.Channel);
                if (!_connectionFuture.Task.isDone())
                {
                    _connectionFuture.SetException(new PulsarClientException("Connection already closed"));
                }
            ...
            {
                return t is Exception || t is ClosedChannelException;
            }
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        _ctx.WriteAndFlushAsync(request).ContinueWith(writeFuture =>
                        {
                            ...
                            {
                                _log.Warning($"{_ctx.Channel} Failed to send request for mutual auth to broker: {writeFuture.Exception.Message}");
            ...
            protected internal override void HandleAckResponse(CommandAckResponse ackResponse)
            {
                ...
                if(_pendingRequests.TryGetValue((long)ackResponse.RequestId, out var future))
                {
                    if (ackResponse?.Error == ServerError.UnknownError && string.IsNullOrWhiteSpace(ackResponse.Message))
                    {
                        future.Requester.Tell(new AckReceipt((long)ackResponse.RequestId));
                    }
                    else
                    {
                        future.Requester.Tell(new AckError((long)ackResponse.RequestId, GetPulsarClientException(ackResponse.Error, ackResponse.Message)));
                    }
            ...
            protected internal override void HandleMessage(CommandMessage cmdMessage, AbstractByteBuffer headersAndPayload)
            {
                ...
                if (_consumers.TryGetValue((long)cmdMessage.ConsumerId, out var consumer))
                {
            ...
            protected internal override void HandleProducerSuccess(CommandProducerSuccess success)
            {
                ...
                if (!success.ProducerReady)
                {
                    ...
                    if (!_pendingRequests.TryGetValue(requestId, out var _))
                    {
                ...
                if (result != null)
                {
                    ...
                    if (firstOneWaiting != null)
                    {
                        ...
                        eventLoopGroup.execute(() =>
                        {
                            ...
                            _ctx.WriteAndFlushAsync(firstOneWaiting.getRight().getLeft()).addListener(writeFuture =>
                            {
            ...
            {
                _log.Warning($"{_ctx.Channel} Received send error from server: {sendError.Error} : {sendError.Message}");
            ...
            protected internal override void HandleCloseConsumer(CommandCloseConsumer closeConsumer)
            {
                ...
                {
                    _log.Warning($"[{remoteAddress}] Consumer with id {consumerId} not found while closing consumer ");
                }
            ...
            public virtual TaskCompletionSource<LookupDataResult> NewLookup(AbstractByteBuffer request, long requestId)
            {
                var future = new TaskCompletionSource<LookupDataResult>();
                ...
                if (_pendingLookupRequestSemaphore.tryAcquire())
                {
                    ...
                    _ctx.WriteAndFlushAsync(request).AddListener(writeFuture =>
                    {
                        if (!writeFuture.isSuccess())
                        {
                ...
                else
                {
                    if (_log.IsDebugEnabled)
                    {
                        _log.Debug($"{requestId} Failed to add lookup-request into pending queue");
                    }

                    if (_maxLookupRequestSemaphore.tryAcquire())
                    {
                        _waitingLookupRequests.add(Pair.of(requestId, Pair.of(request, future)));
                    }
                    else
                    {
                        request.release();
            ...
            public virtual TaskCompletionSource<GetTopicsResult> NewGetTopicsOfNamespace(AbstractByteBuffer request, long requestId)
            {
                return SendRequestAndHandleTimeout(request, requestId, RequestType.GetTopics, true);
            }

            public virtual void NewAckForReceipt(AbstractByteBuffer request, long requestId)
            {
                SendRequestAndHandleTimeout(request, requestId, RequestType.AckResponse);
            }

            public virtual void NewAckForReceiptWithFuture(AbstractByteBuffer request, long requestId, TaskCompletionSource<bool> future)
            {
            ...
            protected internal override void HandleGetTopicsOfNamespaceSuccess(CommandGetTopicsOfNamespaceResponse success)
            {
                ...
                if (_log.IsDebugEnabled)
                {
                    _log.Debug($"{_ctx.Channel} Received get topics of namespace success response from server: {success.RequestId} - topics.size: {success.Topics.Count}");
                }
                ...
                {
                    var msg = $"{_ctx.Channel} Received unknown request id from server: {success.RequestId}";
                    _duplicatedResponseCounter.GetAndIncrement();
                    _log.Warning(msg);
                }
            ...
            protected internal override void HandleGetSchemaResponse(CommandGetSchemaResponse commandGetSchemaResponse)
            {
                ...
                {
                    _duplicatedResponseCounter.GetAndIncrement();
                    var msg = $"{_ctx.Channel} Received unknown request id from server: {requestId}";
                    _log.Warning(msg);
                    requester.Requester.Tell(new AskResponse(Unwrap(new Exception(msg))));
                }
            }
            ...
            protected internal override void HandleGetOrCreateSchemaResponse(CommandGetOrCreateSchemaResponse commandGetOrCreateSchemaResponse)
            {
                ...
                {
                    _duplicatedResponseCounter.GetAndIncrement();
                    _log.Warning($"{_ctx.Channel} Received unknown request id from server: {requestId}");
                }
            }
            ...
            protected internal virtual IChannel Channel()
            {
                return _ctx.Channel;
            }
            ...
            internal virtual ValueTask<ProducerResponse> SendRequestWithId(AbstractByteBuffer cmd, long requestId)
            {
                return SendRequestAndHandleTimeout(cmd, requestId, RequestType.Command, true);
            }

            private void SendRequestAndHandleTimeout<T>(AbstractByteBuffer requestMessage, long requestId, RequestType requestType, bool flush, TaskCompletionSource<T> future)
            {
                if (flush)
                {
                    _ctx.WriteAndFlushAsync(requestMessage).ContinueWith(writeFuture =>
                    {
                        if (!writeFuture.IsCompletedSuccessfully)
                        {
                            _log.Warning($"{_ctx.Channel} Failed to send {requestType.Description} to broker: {writeFuture.Exception.Message}");
                            future.SetException(writeFuture.Exception);
                        }
                    });
                }
                else
                {
                    _ctx.WriteAsync(requestMessage).GetAwaiter();
                }
                _requestTimeoutQueue.Add(new RequestTime(requestId, requestType));
            }

            private TaskCompletionSource<T> SendRequestAndHandleTimeout<T>(AbstractByteBuffer requestMessage, long requestId, RequestType requestType, bool flush)
            {
                var future = new TaskCompletionSource<T>(TaskCreationOptions.RunContinuationsAsynchronously);
                SendRequestAndHandleTimeout(requestMessage, requestId, requestType, flush, future);
                return future;
            }

            public virtual TaskCompletionSource<CommandGetLastMessageIdResponse> SendGetLastMessageId(AbstractByteBuffer request, long requestId)
            {
                return SendRequestAndHandleTimeout(request, requestId, RequestType.GetLastMessageId, true);
            }

            public virtual TaskCompletionSource<Optional<SchemaInfo>> SendGetSchema(AbstractByteBuffer request, long requestId)
            {
            ...
            public virtual TaskCompletionSource<CommandGetSchemaResponse> SendGetRawSchema(AbstractByteBuffer request, long requestId)
            {
                return SendRequestAndHandleTimeout(request, requestId, RequestType.GetSchema, true);
            }

            public virtual TaskCompletionSource<byte[]> SendGetOrCreateSchema(AbstractByteBuffer request, long requestId)
            {
                var future = SendRequestAndHandleTimeout<CommandGetOrCreateSchemaResponse>(request, requestId, RequestType.GetOrCreateSchema, true);
            ...
            public virtual CommandWatchTopicListSuccess NewWatchTopicList(BaseCommand commandWatchTopicList, long requestId)
            {
                if (!_supportsTopicWatchers)
                {
                    return new PulsarClientException.NotAllowedException("Broker does not allow broker side pattern evaluation.");
                }
                return SendRequestAndHandleTimeout(Commands.SerializeWithSize(commandWatchTopicList), requestId, RequestType.Command, true);
            }

            public virtual TaskCompletionSource<CommandSuccess> NewWatchTopicListClose(BaseCommand commandWatchTopicListClose, long requestId)
            {
            ...
            protected internal override void HandleCommandWatchTopicListSuccess(CommandWatchTopicListSuccess commandWatchTopicListSuccess)
            {
                ...
                if (_log.IsDebugEnabled)
                {
                    _log.Debug($"[{_ctx.Channel}] Received watchTopicListSuccess response from server: {commandWatchTopicListSuccess.RequestId}");
                }
                ...
                {
                    _duplicatedResponseCounter.GetAndIncrement();
                    _log.Warning($"[{_ctx.Channel}] Received unknown request id from server: {commandWatchTopicListSuccess.RequestId}");
                }
            }
            ...
            protected internal override void HandleCommandWatchTopicUpdate(CommandWatchTopicUpdate commandWatchTopicUpdate)
            {
                ...
                var watcher = _topicListWatchers[watcherId];
                if (watcher != null)
                {
            ...
            private void CheckServerError(ServerError error, string errMsg)
            {
                ...
                {
                    _log.Error($"{_ctx.Channel} Close connection because received internal-server error {error}");
            ...
            internal virtual void RegisterConsumer<T1>(long consumerId, IActorRef consumer)
            {
            ...
            internal virtual void RemoveTopicListWatcher(long watcherId)
            {
