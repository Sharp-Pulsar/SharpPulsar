...
namespace SharpPulsar.Benchmarks.Bench
{
    ...
    public class Producers500000
    {
        ...
        [Benchmark]
        public void A_500_000_producers()
        {
            for (var i = 0; i < 500_000; i++)
            {
                var p = _client.NewProducer(new ProducerConfigBuilder<byte[]>()
                    .Topic($"A_500_000_producers-{i}-{Guid.NewGuid()}"));
                var data = Encoding.UTF8.GetBytes($"A_500_000_producers-{i}");
                var id = p.NewMessage().Value(data).Send();
                Console.WriteLine($"A_500_000_producers({id.LedgerId}:{id.EntryId})");
            }
            Console.WriteLine("500,000 producers");
        }
