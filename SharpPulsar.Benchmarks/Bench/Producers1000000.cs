...
namespace SharpPulsar.Benchmarks.Bench
{
    ...
    public class Producers1000000
    {
        ...
        [Benchmark]
        public void A_1_000_000_producers()
        {
            for (var i = 0; i < 1_000_000; i++)
            {
                var p = _client.NewProducer(new ProducerConfigBuilder<byte[]>()
                    .Topic($"A_1_000_000_producers-{i}-{Guid.NewGuid()}"));
                var data = Encoding.UTF8.GetBytes($"A_1_000_000_producers-{i}");
                var id = p.NewMessage().Value(data).Send();
                Console.WriteLine($"A_1_000_000_producers({id.LedgerId}:{id.EntryId})");
            }
            Console.WriteLine("1,000,000 producers");
        }
    }
}
