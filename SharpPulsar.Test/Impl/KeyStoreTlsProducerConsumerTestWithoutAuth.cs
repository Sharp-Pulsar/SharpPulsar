...
{
    public class KeyStoreTlsProducerConsumerTestWithoutAuth : ProducerConsumerBase
	{
		...
		internal ISet<string> tlsProtocols = Sets.newConcurrentHashSet();
		...
		public virtual void internalSetUpForClient(bool addCertificates, string lookupUrl)
		{
			...
			ClientBuilder clientBuilder = PulsarClient.builder().serviceUrl(lookupUrl).enableTls(true).useKeyStoreTls(true).tlsTrustStorePath(BROKER_TRUSTSTORE_FILE_PATH).tlsTrustStorePassword(BROKER_TRUSTSTORE_PW).allowTlsInsecureConnection(false).operationTimeout(1000, TimeUnit.MILLISECONDS);
			if (addCertificates)
			{
		...
		public virtual void internalSetUpForNamespace()
		{
			...
			admin = spy(PulsarAdmin.builder().serviceHttpUrl(brokerUrlTls.ToString()).useKeyStoreTls(true).tlsTrustStorePath(BROKER_TRUSTSTORE_FILE_PATH).tlsTrustStorePassword(BROKER_TRUSTSTORE_PW).allowTlsInsecureConnection(true).authentication(typeof(AuthenticationKeyStoreTls).FullName, authParams).build());
