...
namespace Org.Apache.Pulsar.Client.Impl.Schema
{
	public class AvroSchemaTest
	{
		...
		{
			var reader = new JsonReader<Foo>(new ObjectMapper(), typeof(Foo));
			var writer = new JsonWriter<Foo>(new ObjectMapper());
			var schemaDefinition = SchemaDefinition.builder<Foo>().withPojo(typeof(Bar)).withSchemaReader(reader).withSchemaWriter(writer).build();
