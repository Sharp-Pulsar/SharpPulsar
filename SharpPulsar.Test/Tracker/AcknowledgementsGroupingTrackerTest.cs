...
namespace SharpPulsar.Test.Tracker
{
	...
	public class AcknowledgementsGroupingTrackerTest
	{
		...
		public void TestAckTracker()
		{
			...
			var isDuplicate = tracker.AskFor<bool>(new IsDuplicate(msg1));
			Assert.False(isDuplicate);
			tracker.Tell(new AddAcknowledgment(msg1, CommandAck.AckType.Individual, new Dictionary<string, long>(), null));
			isDuplicate = tracker.AskFor<bool>(new IsDuplicate(msg1));
			Assert.True(isDuplicate);

			isDuplicate = tracker.AskFor<bool>(new IsDuplicate(msg2));
			Assert.False(isDuplicate);
			...
			isDuplicate = tracker.AskFor<bool>(new IsDuplicate(msg2));
			Assert.True(isDuplicate);
			isDuplicate = tracker.AskFor<bool>(new IsDuplicate(msg3));
			Assert.True(isDuplicate);

			isDuplicate = tracker.AskFor<bool>(new IsDuplicate(msg4));
			Assert.True(isDuplicate);
			isDuplicate = tracker.AskFor<bool>(new IsDuplicate(msg5));
			Assert.True(isDuplicate);
			isDuplicate = tracker.AskFor<bool>(new IsDuplicate(msg6));
			Assert.False(isDuplicate);
