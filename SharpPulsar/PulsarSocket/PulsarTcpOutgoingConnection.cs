...
namespace SharpPulsar.PulsarSocket
{
    ...
    /// TBD
    /// </summary>
    internal sealed class PulsarTcpOutgoingConnection : PulsarTcpConnection
    {
        ...
        public PulsarTcpOutgoingConnection(TcpExt tcp, IActorRef commander, PulsarTcp.Connect connect)
            : base(
                   tcp,
                   ...
                   tcp.Settings.WriteCommandsQueueMaxSize >= 0 ? tcp.Settings.WriteCommandsQueueMaxSize : Option<int>.None)
        {
            ...
            _connect = connect;
            var poolOption =
                connect.Options.OfType<ByteBufferPoolSize>().FirstOrDefault();
        ...
        private void Register(IPEndPoint address, IPEndPoint fallbackAddress)
        {
            ReportConnectFailure(() =>
            {
                ...
                if (!Socket.ConnectAsync(_connectArgs))
                    Self.Tell(PulsarTcp.SocketConnected.Instance);
