...
{
    using static SharpPulsar.PulsarSocket.PulsarTcp;
    ...
    /// </summary>
    internal abstract class PulsarTcpConnection : ActorBase, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
    {
        [Flags]
        enum ConnectionStatus
        {
            ...
            /// <see cref="ReceiveArgs"/> are currently trying to receive data.
            /// </summary>
            Receiving = 1,

            /// <summary>
            /// Marks that connection has invoked <see cref="Socket.SendAsync"/> and that 
            /// <see cref="SendArgs"/> are currently sending data. It's important as 
            ...
            /// i.e. because underlying <see cref="SendArgs"/> is actually sending the data.
            /// </summary>
        ...
        protected PulsarTcpConnection(TcpExt tcp, Socket socket, bool pullMode, Option<int> writeCommandsBufferMaxSize)
        {
        ...
        private ReadResult InnerRead(ConnectionInfo info, int remainingLimit, SocketAsyncEventArgs ea)
        {
            if (remainingLimit > 0)
            {
                ...
                if (ea.SocketError == SocketError.Success && readBytes > 0)
                    info.Handler.Tell(new Received(PulsarByteString.CopyFrom(ea.Buffer, ea.Offset, ea.BytesTransferred)));
        ...
        {
            private readonly PulsarTcpConnection _connection;
            ...
            public PendingBufferWrite(
                PulsarTcpConnection connection,
