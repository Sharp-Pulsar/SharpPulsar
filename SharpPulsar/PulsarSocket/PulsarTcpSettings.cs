...
namespace SharpPulsar.PulsarSocket
{
    ...
    /// TBD
    /// </summary>
    public class PulsarTcpSettings
    {
        /// <summary>
        /// Creates a new instance of <see cref="PulsarTcpSettings"/> class 
        /// and fills it with values parsed from `akka.io.tcp` HOCON
        /// path found in actor system.
        /// </summary>
        public static PulsarTcpSettings Create(ActorSystem system)
        {
            var config = system.Settings.Config.GetConfig("akka.io.tcp");
            if (config.IsNullOrEmpty())
                throw ConfigurationException.NullOrEmptyConfig<PulsarTcpSettings>("akka.io.tcp");//($"Failed to create {typeof(TcpSettings)}: akka.io.tcp configuration node not found");
        ...
        public static PulsarTcpSettings Create(Config config)
        {
            if (config.IsNullOrEmpty())
                throw ConfigurationException.NullOrEmptyConfig<PulsarTcpSettings>();

            return new PulsarTcpSettings(
        ...
        public PulsarTcpSettings(string bufferPoolConfigPath,
        ...
        /// <summary>
        /// Limits maximum size of internal queue, used in <see cref="PulsarTcpIncomingConnection"/> connection actor
