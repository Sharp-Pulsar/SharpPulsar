...
namespace SharpPulsar.PulsarSocket
{
    ...
    ///   copied to a separate <see cref="PulsarByteString"/> object (so it's NOT a zero-copy operation).
    ...
    ///   <see cref="PulsarByteString"/> object). Therefore writes don't allocate any byte buffers.
    /// 
    ...
    /// </summary>
    internal abstract class PulsarTlsConnection : ActorBase, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
    {
        [Flags]
        enum ConnectionStatus
        {
            ...
            /// <see cref="PulsarTcpConnection.ReceiveArgs"/> are currently trying to receive data.
            /// </summary>
            Receiving = 1,

            /// <summary>
            /// Marks that connection has invoked <see cref="Socket.SendAsync"/> and that 
            /// <see cref="PulsarTcpConnection.SendArgs"/> are currently sending data. It's important as 
            ...
            /// i.e. because underlying <see cref="PulsarTcpConnection.SendArgs"/> is actually sending the data.
            /// </summary>
        ...
        protected PulsarTlsConnection(TcpExt tcp, Socket socket, PulsarTlsConnectionOption opt, bool pullMode, Option<int> writeCommandsBufferMaxSize)
        {
        ...
        public PulsarTlsConnectionOption TlsOptions { get; protected set; }
        ...
        private Receive WaitingForRegistration(IActorRef commander)
        {
            return message =>
            {
                switch (message)
                {
                    case PulsarTcp.Register register:
                        ...
                        return true;
                    case PulsarTcp.ResumeReading _: ClearStatus(ConnectionStatus.ReadingSuspended); return true;
                    case PulsarTcp.SuspendReading _: SetStatus(ConnectionStatus.ReadingSuspended); return true;
                    case PulsarTcp.CloseCommand cmd:
                    ...
                    case ReceiveTimeout _:
                        ...
                        Context.Stop(Self);
                        return true;
                    case PulsarTcp.WriteCommand write:
        ...
        private Receive Connected(ConnectionInfo info)
        {
            ...
            return message =>
            {
                ...
                switch (message)
                {
                    case PulsarTcp.SuspendReading _: SuspendReading(); return true;
                    case PulsarTcp.ResumeReading _: ResumeReading(); return true;
                    case PulsarTcp.SocketReceived _: DoRead(info, null); return true;
                    case PulsarTcp.CloseCommand cmd: HandleClose(info, Sender, cmd.Event); return true;
                    default: return false;
                }
            };
        }
        ...
        private Receive PeerSentEOF(ConnectionInfo info)
        {
            ...
            {
                if (handleWrite(message)) return true;
                var cmd = message as PulsarTcp.CloseCommand;
                if (cmd != null)
                {
                    HandleClose(info, Sender, cmd.Event);
                    return true;
                }
                if (message is PulsarTcp.ResumeReading) return true;
                return false;
            };
        }
        ...
        /// </summary>
        private Receive ClosingWithPendingWrite(ConnectionInfo info, IActorRef closeCommander, PulsarTcp.ConnectionClosed closedEvent)
        {
            return message =>
            {
                switch (message)
                {
                    case PulsarTcp.SuspendReading _: SuspendReading(); return true;
                    case PulsarTcp.ResumeReading _: ResumeReading(); return true;
                    case PulsarTcp.SocketReceived _: DoRead(info, closeCommander); return true;
                    case PulsarTcp.SocketSent _:
                        AcknowledgeSent();
                        if (IsWritePending)
                    ...
                    case PulsarTcp.Abort _: HandleClose(info, Sender, PulsarTcp.Aborted.Instance); return true;
                    default: return false;
                }
            };
        }
            ...
            return message =>
            {
                switch (message)
                {
                    case PulsarTcp.SocketSent _:
                        ...
                        {
                            _interestedInResume.Tell(PulsarTcp.WritingResumed.Instance);
                    ...
                    case PulsarTcp.WriteCommand write:
                        ...
                        return true;
                    case PulsarTcp.ResumeWriting _:
                        /*
                        ...
                        if (IsWritePending)
                        {
                            ...
                            else Sender.Tell(new PulsarTcp.CommandFailed(PulsarTcp.ResumeWriting.Instance));
                        }
                        else Sender.Tell(PulsarTcp.WritingResumed.Instance);
                        return true;
                    case UpdatePendingWriteAndThen updatePendingWrite:
        ...
        private void DropWrite(ConnectionInfo info, PulsarTcp.WriteCommand write)
        {
        ...
        protected void CompleteConnect(IActorRef commander, IEnumerable<Inet.SocketOption> options)
        {
            ...
            commander.Tell(new PulsarTcp.Connected(Socket.RemoteEndPoint, Socket.LocalEndPoint));
            Authenticate();
                    ...
                    switch (read.Type)
                    {
                        ...
                        case ReadResultType.EndOfStream:
                            if (_isOutputShutdown)
                            {
                                ...
                                DoCloseConnection(info, closeCommander, PulsarTcp.ConfirmedClosed.Instance);
                            }
                            else
                            {
                                if (_traceLogging) Log.Debug("Read returned end-of-stream, our side not yet closed");
                                HandleClose(info, closeCommander, PulsarTcp.PeerClosed.Instance);
                            }
                            break;
        ...
        private ReadResult InnerRead(ConnectionInfo info, int remainingLimit, SendReceiveArgsFake ea)
        {
            if (remainingLimit > 0)
            {
                ...
                if (ea.SocketError == SocketError.Success && readBytes > 0)
                    info.Handler.Tell(new PulsarTcp.Received(PulsarByteString.CopyFrom(ea.Buffer, ea.Offset, ea.BytesTransferred)));
        ...
        private void DoWrite(ConnectionInfo info, Option<PendingWrite> write)
        {
            ...
            foreach (var pendingAck in write.Value.PendingAcks.Where(ackInfo => !ackInfo.Ack.Equals(PulsarTcp.NoAck.Instance)))
            {
        ...
        private void HandleClose(ConnectionInfo info, IActorRef closeCommander, PulsarTcp.ConnectionClosed closedEvent)
        {
            SetStatus(ConnectionStatus.ShutdownRequested);

            if (closedEvent is PulsarTcp.Aborted)
            {
                if (_traceLogging) Log.Debug("Got Abort command. RESETing connection.");
                DoCloseConnection(info, closeCommander, closedEvent);
            }
            else if (closedEvent is PulsarTcp.PeerClosed && info.KeepOpenOnPeerClosed)
            {
                // report that peer closed the connection
                info.Handler.Tell(PulsarTcp.PeerClosed.Instance);
            ...
            else if (closedEvent is PulsarTcp.ConfirmedClosed) // shutdown output and wait for confirmation
            {
        ...
        private void DoCloseConnection(ConnectionInfo info, IActorRef closeCommander, PulsarTcp.ConnectionClosed closedEvent)
        {
            if (closedEvent is PulsarTcp.Aborted) Abort();
            else
            {
                CloseSocket();
            }
        ...
        private void HandleError(IActorRef handler, SocketException exception)
        {
            ...
            StopWith(new CloseInformation(new HashSet<IActorRef>(new[] { handler }), new PulsarTcp.ErrorClosed(exception.Message)));
        }
        ...
        public void PushRead(byte[] bytes, int numBytesRead, IActorRef onCompleteNotificationsReceiver)
        {
            ...
            onCompleteNotificationsReceiver.Tell(PulsarTcp.SocketReceived.Instance);
        }
            ...
            if (!HasStatus(ConnectionStatus.Receiving))
            {
                SslStream.ReadAsync(ReceiveArgs.Buffer, ReceiveArgs.Offset,
                    ReceiveArgs.Count).PipeTo(Self, Self, i =>
                 {
                     ...
                     return PulsarTcp.SocketReceived.Instance;
                 }, e =>
                 {
        ...
        private Option<PendingWrite> GetNextWrite(IEnumerable<(PulsarTcp.SimpleWriteCommand Command, IActorRef Sender)> headCommands = null)
        {
            headCommands = headCommands ?? ImmutableList<(PulsarTcp.SimpleWriteCommand Command, IActorRef Sender)>.Empty;
            var writeCommands = new List<(PulsarTcp.Write Command, IActorRef Sender)>(_writeCommandsQueue.ItemsCount);
            foreach (var commandInfo in headCommands.Concat(_writeCommandsQueue.DequeueAll()))
            {
                switch (commandInfo.Command)
                {
                    case PulsarTcp.Write w when !w.Data.IsEmpty:
                        // Have real write - go on and put it to the wire
                        writeCommands.Add((w, commandInfo.Sender));
                        break;
                    case PulsarTcp.Write w:
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private PendingWrite CreatePendingBufferWrite(List<(PulsarTcp.Write Command, IActorRef Sender)> writes)
        {
        ...
        protected sealed class CloseInformation
        {
            ...
            public PulsarTcp.Event ClosedEvent { get; }

            public CloseInformation(ISet<IActorRef> notificationsTo, PulsarTcp.Event closedEvent)
            {
        ...
        {
            private readonly PulsarTlsConnection _connection;
            private readonly IActorRef _self;
            private readonly IEnumerable<PulsarByteString> _dataToSend;
            private readonly IBufferPool _bufferPool;
            private readonly SendReceiveArgsFake _sendArgs;

            public PendingBufferWrite(
                PulsarTlsConnection connection,
                SendReceiveArgsFake sendArgs,
                IActorRef self,
                IImmutableList<(IActorRef Commander, object Ack)> acks,
                IEnumerable<PulsarByteString> dataToSend,
...
#pragma warning disable 4014
                    Task.Run(async () =>
                    {
                        foreach (var byteString in _dataToSend)
                        {
                            ...
                            {
                                await _connection.SslStream.WriteAsync(buffer.Array,
...
#pragma warning restore 4014
                    {
                        return PulsarTcp.SocketSent.Instance;
                    }, e =>
                    {
        ...
        public class PendingSimpleWritesQueue
        {
            ...
            private readonly Queue<(PulsarTcp.SimpleWriteCommand Command, IActorRef Commander, int Size)> _queue;
            ...
            public PendingSimpleWritesQueue(ILoggingAdapter log, Option<int> maxQueueSizeInBytes)
            {
                ...
                _queue = new Queue<(PulsarTcp.SimpleWriteCommand Command, IActorRef Commander, int Size)>();
            }
            ...
            /// <summary>
            /// Adds all <see cref="PulsarTcp.SimpleWriteCommand"/> subcommands stored in provided command.
            ...
            /// </exception>
            public bool EnqueueSimpleWrites(PulsarTcp.WriteCommand command, IActorRef sender)
            {
            ...
            /// Thrown when data to buffer is larger then allowed <see cref="_maxQueueSizeInBytes"/>
            /// </exception>
            public bool EnqueueSimpleWrites(PulsarTcp.WriteCommand command, IActorRef sender, out int bufferedSize)
            {
                bufferedSize = 0;
            ...
            /// </summary>
            /// <returns>
            /// Not buffered (and not checked) first <see cref="PulsarTcp.SimpleWriteCommand"/>
            /// </returns>
            /// <exception cref="InternalBufferOverflowException">
            /// Thrown when data to buffer is larger then allowed <see cref="_maxQueueSizeInBytes"/>
            /// </exception>
            public bool EnqueueSimpleWritesExceptFirst(PulsarTcp.WriteCommand command, IActorRef sender, out PulsarTcp.SimpleWriteCommand first)
            {
                first = null;
                foreach (var writeInfo in ExtractFromCommand(command))
                {
            ...
            /// </summary>
            public (PulsarTcp.SimpleWriteCommand, IActorRef Sender) Dequeue()
            {
                if (_queue.Count == 0)
            ...
            /// </summary>
            /// <returns></returns>
            public IEnumerable<(PulsarTcp.SimpleWriteCommand Command, IActorRef Sender)> DequeueAll()
            {
            ...
            /// Gets next command from the queue, if any
            /// </summary>
            public bool TryGetNext(out (PulsarTcp.SimpleWriteCommand Command, IActorRef Sender) command)
            {
                command = default;
                if (_queue.Count == 0)
                    return false;
            ...
            private IEnumerable<(PulsarTcp.SimpleWriteCommand Command, int DataSize)> ExtractFromCommand(PulsarTcp.WriteCommand command)
            {
                switch (command)
                {
                    case PulsarTcp.Write write:
                        yield return (write, write.Data.Count);
                        break;
                    case PulsarTcp.CompoundWrite compoundWrite:
