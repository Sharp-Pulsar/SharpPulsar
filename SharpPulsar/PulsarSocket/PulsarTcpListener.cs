...
{
    partial class PulsarTcpListener : ActorBase, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
    {
        private readonly TcpExt _tcp;
        private readonly IActorRef _bindCommander;
        private readonly PulsarTcp.Bind _bind;
        ...
        public PulsarTcpListener(TcpExt tcp, IActorRef bindCommander,
            PulsarTcp.Bind bind)
        {
            _tcp = tcp;
            ...
            bindCommander.Tell(new PulsarTcp.Bound(_socket.LocalEndPoint));
        }
        ...
        {
            return PulsarTcp.ConnectionSupervisorStrategy;
        }
        ...
        protected override bool Receive(object message)
        {
            if (message is SocketAsyncEventArgs)
            {
                ...
                if (saea.SocketError == SocketError.Success)
                    Context.ActorOf(Props.Create<PulsarTcpIncomingConnection>(_tcp, saea.AcceptSocket, _bind.Handler, _bind.Options, _bind.PullMode).WithDispatcherIfNeeded(_bind.Options));
            ...
            var resumeAccepting = message as PulsarTcp.ResumeAccepting;
            ...
            if (message is PulsarTcp.Unbind)
            {
                _log.Debug("Unbinding endpoint {0}", _bind.LocalAddress);
                _socket.Dispose();
                Sender.Tell(PulsarTcp.Unbound.Instance);
