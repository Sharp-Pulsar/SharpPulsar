...
namespace Pulsar.Api
{
	...
	/// </summary>
	[Obsolete]
	public interface IProducerInterceptor<T> : IAsyncDisposable
	{
		/// <summary>
		...
		/// interceptor's <seealso cref="beforeSend(IProducer, IMessage)"/> callback will be called in the
		/// order specified by
		/// <seealso cref="IProducerBuilder.intercept(IProducerInterceptor[])"/>.
		/// 
		/// </para>
		/// <para>The first interceptor in the list gets the message passed from the client,
		...
		IMessage<T> BeforeSend(IProducer<T> producer, IMessage<T> message);
		...
		void OnSendAcknowledgement(IProducer<T> producer, IMessage<T> message, IMessageId msgId, Exception exception);
