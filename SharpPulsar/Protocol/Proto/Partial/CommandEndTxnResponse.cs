...
{
	public sealed partial class CommandEndTxnResponse : ByteBufCodedOutputStream.ByteBufGeneratedMessage
	{
		// Use CommandEndTxnResponse.newBuilder() to construct.
		internal static ThreadLocalPool<CommandEndTxnResponse> _pool = new ThreadLocalPool<CommandEndTxnResponse>(handle => new CommandEndTxnResponse(handle), 1, true);

		internal ThreadLocalPool.Handle _handle;
		private CommandEndTxnResponse(ThreadLocalPool.Handle handle)
		{
			_handle = handle;
		}
		...
		public CommandEndTxnResponse(bool NoInit)
		{
		}
		...
		{
			// Construct using org.apache.pulsar.common.api.proto.CommandEndTxnResponse.newBuilder()
			...
			public CommandEndTxnResponse BuildParsed()
			{
				CommandEndTxnResponse Result = BuildPartial();
			...
			{
				CommandEndTxnResponse Result = CommandEndTxnResponse._pool.Take();
		...
		{
			_defaultInstance = new CommandEndTxnResponse(true);
