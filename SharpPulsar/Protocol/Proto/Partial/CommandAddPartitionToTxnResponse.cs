...
{
	public sealed partial class CommandAddPartitionToTxnResponse : ByteBufCodedOutputStream.ByteBufGeneratedMessage
	{
		// Use CommandAddPartitionToTxnResponse.newBuilder() to construct.
		internal static ThreadLocalPool<CommandAddPartitionToTxnResponse> _pool = new ThreadLocalPool<CommandAddPartitionToTxnResponse>(handle => new CommandAddPartitionToTxnResponse(handle), 1, true);

		internal ThreadLocalPool.Handle _handle;
		private CommandAddPartitionToTxnResponse(ThreadLocalPool.Handle handle)
		{
			_handle = handle;
		}
		public void Recycle()
		...
		public CommandAddPartitionToTxnResponse(bool NoInit)
		{
		}

		internal static readonly CommandAddPartitionToTxnResponse _defaultInstance;
		public static CommandAddPartitionToTxnResponse DefaultInstance => _defaultInstance;

        public CommandAddPartitionToTxnResponse DefaultInstanceForType => _defaultInstance;
		...
		{
			// Construct using org.apache.pulsar.common.api.proto.CommandAddPartitionToTxnResponse.newBuilder()
			...
			public CommandAddPartitionToTxnResponse DefaultInstanceForType => DefaultInstance;
			...
			public CommandAddPartitionToTxnResponse BuildParsed()
			{
				CommandAddPartitionToTxnResponse Result = BuildPartial();
				if (!Result.Initialized)
				{
			...
			{
				CommandAddPartitionToTxnResponse Result = CommandAddPartitionToTxnResponse._pool.Take();
		...
		{
			_defaultInstance = new CommandAddPartitionToTxnResponse(true);
