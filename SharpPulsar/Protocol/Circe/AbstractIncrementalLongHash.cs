...
namespace SharpPulsar.Protocol.Circe
{
	...
	/// </summary>
	public abstract class AbstractIncrementalLongHash : IncrementalLongHash
	{
		...
		public long Calculate(IByteBuffer input)
		{
			return Resume(Initial(), input);
		}
		...
		public long Resume(long current, sbyte[] input)
		{
			return ResumeUnchecked(current, input, 0, input.Length);
		}

		public long Resume(long current, sbyte[] input, int index, int length)
		{
			if (length < 0)
			{
		...
		public long Resume(long current, IByteBuffer input)
		{
			sbyte[] array;
			int index;
		...
		public abstract long ResumeUnchecked(long current, sbyte[] input, int index, int length);
	}
