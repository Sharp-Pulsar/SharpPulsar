...
namespace SharpPulsar.EventSource.Messages.Pulsar
{
    ...
    public sealed class CurrentEventsByTag<T> : IPulsarEventSourceMessage<T>
    {
        public CurrentEventsByTag(string tenant, string ns, string topic, long fromSequenceId, long toSequenceId, Tag tag, string adminUrl, ReaderConfigurationData<T> configuration, ClientConfigurationData clientConfiguration)
        {
            Tenant = tenant;
            Namespace = ns;
