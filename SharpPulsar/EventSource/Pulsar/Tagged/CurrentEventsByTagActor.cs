...
namespace SharpPulsar.EventSource.Pulsar.Tagged
{
    public class CurrentEventsByTagActor : ReceiveActor
    {
        ...
        public CurrentEventsByTagActor(CurrentEventsByTag message, HttpClient httpClient, IActorRef client, IActorRef lookup, IActorRef cnxPool, IActorRef generator)
        {
            _message = message;
        ...
        private void Setup(Partitions p)
        {
            if (p.Partition > 0)
            {
                for (var i = 0; i < p.Partition; i++)
                {
                    ...
                    var child = Context.ActorOf(PulsarTaggedSourceActor.Prop(_message.ClientConfiguration, config, _pulsarManager, _network, msgId.End, false, _httpClient, _message, _message.Tag, msgId.Start.Index));
                    Context.Watch(child);
                }
            }
            else
