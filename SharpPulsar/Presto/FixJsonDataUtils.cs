...
namespace SharpPulsar.Presto
{
	public sealed class FixJsonDataUtils
	{
		...
		public static IList<object> FixData(IList<Column> columns, IEnumerable<IList<object>> data)
		{
			...
			Condition.RequireNonNull(columns, "columns", "columns is null");
			//import static com.facebook.presto.spi.type.TypeSignature.parseTypeSignature;
			IList<TypeSignature> signatures = columns.ToList().Select(column => TypeSignature.ParseTypeSignature(column.Type)).ToList();
			IList<object> rows = new List<object>();
			foreach (IList<object> row in data)
			{ 
                Condition.CheckArgument(row.Count == columns.Count, "row/column size mismatch");
		...
		private static object FixValue(TypeSignature signature, object value)
		{
			...
			if (signature.Base.Equals(StandardTypes.Array))
			{
				...
				{
					fixedValue.Add(FixValue(signature.TypeParametersAsTypeSignatures.ToList()[0], @object));
				}
				return fixedValue;
			}
			if (signature.Base.Equals(StandardTypes.Map))
			{
				TypeSignature keySignature = signature.TypeParametersAsTypeSignatures.ToList()[0];
				TypeSignature valueSignature = signature.TypeParametersAsTypeSignatures.ToList()[1];
			...
			if (signature.Base.Equals(StandardTypes.Row))
			{
				...
				Condition.CheckArgument(listValue.Count == signature.Parameters.Count(), "Mismatched data values and row type");
				for (int i = 0; i < listValue.Count; i++)
				{
					TypeSignatureParameter parameter = signature.Parameters.ToList()[i];
					Condition.CheckArgument(parameter.Kind == ParameterKind.NamedType, "Unexpected parameter [%s] for row type", parameter);
